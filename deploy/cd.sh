...
deploy_onap() {
  ...
  echo "provide onap-parameters.yaml(amsterdam) or dev0/dev1.yaml(master) and aai-cloud-region-put.json"
  ...
  if [ "$BRANCH" == "amsterdam" ]; then
    ...
    echo "sleep ${DELAY} sec to allow base platform pods to complete"
    sleep $DELAY
