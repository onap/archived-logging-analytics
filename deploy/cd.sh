...
# source from https://jira.onap.org/browse/OOM-320, 326, 321, 898
# Michael O'Brien
#
...
deploy_onap() {
  ...
  if [ "$BRANCH" == "amsterdam" ]; then
    ...
    if [[ "$FULL_MANAGED_DEPLOY" != true ]]; then
      ...
      if [[ "$BRANCH" == "casablanca" ]] || [[ "$BRANCH" == "3.0.0-ONAP" ]] || [[ "$BRANCH" == "3.0.1-ONAP" ]]; then 
        #DEPLOY_ORDER_POD_NAME_ARRAY=('robot' 'consul' 'aaf' 'dmaap' 'dcaegen2' 'msb' 'aai' 'esr' 'multicloud' 'oof' 'so' 'sdc' 'sdnc' 'vid' 'policy' 'portal')
        DEPLOY_ORDER_POD_NAME_ARRAY=('robot consul aaf dmaap dcaegen2 msb aai esr multicloud oof so sdc sdnc vid policy portal log vfc uui vnfskd appc clamp cli pomba vvp contrib sniro')
      ...
      for POD_NAME in $DEPLOY_ORDER_POD_NAME_ARRAY; do
        ...
        #sleep $SPLIT_DEPLOY_DELAY
        wait_for_pod $POD_NAME- $PODS_PARTIAL  
     done
    fi
    cd ../../
  fi
