...
class MDCFormatter(logging.Formatter):
    ...
    def __init__(self, fmt=None, mdcfmt=None, datefmt=None, style="%"):
        ...
        if sys.version_info > (3, 2):
            super(MDCFormatter, self).__init__(fmt=fmt, datefmt=datefmt, style=style)
    ...
    def format(self, record):
        ...
        if mdcFmtWords is None:
            if sys.version_info > (3, 2):
                self._style = logging._STYLES[self.style][0](self._fmt.replace(self._mdc_tag, ""))
            else:
        ...
        try:
            ...
            if sys.version_info > (3, 2):
                self._style = logging._STYLES[self.style][0](self._fmt.replace(self._mdc_tag, mdcstr))
            else:
